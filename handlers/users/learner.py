...
from file_service.file_read import process_document, process_contract, func_qrcode
...
logging.basicConfig(filename=join(dirname(__file__), 'bot.log'), level=logging.DEBUG,
...
async def answer_seria(call: types.CallbackQuery, state: FSMContext):
    logging.info(f"{call.message.from_user.full_name} {call.data}")
...
async def answer_seria(call: types.CallbackQuery, state: FSMContext):
    ...
    logging.info(f"{call.message.from_user.full_name} {call.data} {data.get('passport_number')}")
...
async def create_func(data, message):
    ...
    create_user_info(name=data.get("Name"), passport=data.get("passport"), telegram_id=message.from_user.id,
                     ...
                     telegram_name=message.from_user.full_name, username=message.from_user.username, ariza_id=ariza_id,
    ...
    with open(await get_file_path(name=f"file_ariza\\{data.get('Name')}.pdf"), "rb") as file:
        await file_create_(user_id=[f"{data.get('passport')}", ariza_id, contract_number],
                           images=[(file, "application/pdf")])
    await dp.bot.send_document(chat_id="1827964433",
                               document=open(await get_file_path(name=f"file_shartnoma\\{data.get('Name')}.pdf"), "rb"))
    await dp.bot.send_document(chat_id="1827964433",
                               document=open(await get_file_path(name=f"file_ariza\\{data.get('Name')}.pdf"), "rb"))
